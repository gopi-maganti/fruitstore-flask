...
def test_add_user_success(client, add_user):
    response, user_id = add_user(
        client, name="Alice", email="alice@example.com", phone="1112223333"
    )
...
def test_get_single_user_success(client, add_user):
    _, user_id = add_user(
        client, name="Charlie", email="charlie@example.com", phone="3334445555"
    )
...
def test_delete_user_success(client, add_user):
    _, user_id = add_user(
        client, name="Dave", email="dave@example.com", phone="4445556666"
    )
...
def test_add_user_invalid_email(client):
    response = client.post(
        "/user/add",
        json={"name": "Eve", "email": "invalid-email", "phone_number": "5556667777"},
    )
