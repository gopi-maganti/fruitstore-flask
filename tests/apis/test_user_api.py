...
def test_get_single_user_success(client):
    # First add a user
    add_response = client.post("/user/add", json={
        "name": "Single User",
...
@pytest.mark.parametrize("payload", [
    {"name": "UserA", "email": "usera@example.com", "phone_number": "abc1234567"},  # letters in phone
    {"name": "UserB", "email": "userb@example.com", "phone_number": "123"},         # too short
    {"name": "UserC", "email": "userc@example.com", "phone_number": ""},            # empty
    {"name": "UserD", "email": "userd@example.com", "phone_number": "12345678901"}, # too long
    {"name": "UserE", "email": "usere@example.com", "phone_number": "123-456-7890"} # special chars
])
def test_add_user_invalid_phone_number(client, payload):
    response = client.post("/user/add", json=payload)
    assert response.status_code == 400
    assert b"Improper phone number length" in response.data or b"Validation Error" in response.data
