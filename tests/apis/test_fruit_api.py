...
def test_get_all_fruits_success(client):
    ...
    data = {k: v for k, v in payload.items() if k != "image"}
    file_data = {"image": image_data}
    client.post("/fruit/add", data={**data, **file_data}, content_type='multipart/form-data')
...
def test_delete_fruit_success(client):
    ...
    delete_response = client.delete(f"/fruit/{fruit_id}")
...
@pytest.mark.parametrize("payload, expected_error", [
    # Missing sell_by_date (already covered)
    (
        {
    ...
    # All fields empty (already covered)
    (
        {
    ...
    # Invalid weight type (string instead of float)
    (
        {
            "name": "Tomato", "description": "Red fruit", "color": "Red", "size": "Small", "has_seeds": "true",
            "weight": "heavy", "price": "1.0", "total_quantity": "30", "available_quantity": "20", "sell_by_date": "2030-10-10"
        },
        b"Invalid number field"
    ),
    ...
    (
        {
            "name": "Mango", "description": "Tropical fruit", "color": "Yellow", "size": "Large", "has_seeds": "true",
            "weight": "0.5", "price": "1.8", "total_quantity": "-10", "available_quantity": "5", "sell_by_date": "2030-10-10"
        },
    ...
    (
        {
            "name": "Lemon", "description": "Sour", "color": "Yellow", "size": "Small", "has_seeds": "true",
            "weight": "0.2", "price": "cheap", "total_quantity": "50", "available_quantity": "30", "sell_by_date": "2030-09-01"
        },
        b"Invalid number field"
    ),
    ...
    (
        {
            "name": "Guava", "description": "Sweet and green", "color": "Green", "size": "Medium", "has_seeds": "true",
            "weight": "0.3", "price": "1.2", "total_quantity": "50", "available_quantity": "50", "sell_by_date": "2020-01-01"
        },
    ...
    (
        {
            "description": "Missing name", "color": "Red", "size": "Small", "has_seeds": "true",
            "weight": "0.2", "price": "1.1", "total_quantity": "10", "available_quantity": "10", "sell_by_date": "2030-01-01"
        },
        b"field required"
    ),
    ...
    (
        {
            "name": "Peach", "description": "Fuzzy", "color": "Pink", "size": "Medium", "has_seeds": "maybe",
            "weight": "0.3", "price": "1.3", "total_quantity": "20", "available_quantity": "15", "sell_by_date": "2030-10-10"
        },
...
def test_add_fruit_duplicate(client):
    image_data = (io.BytesIO(b"fake image data"), "pear.jpg")
    payload = {
        "name": "Pear",
    ...
    data = {k: v for k, v in payload.items() if k != "image"}
    file_data = {"image": payload["image"]}

    # First POST should work
    client.post("/fruit/add", data={**data, **file_data}, content_type="multipart/form-data")

    # Second identical POST should fail
    response = client.post("/fruit/add", data={**data, **file_data}, content_type="multipart/form-data")
    assert response.status_code == 400
...
def test_delete_fruit_not_found(client):
    response = client.delete("/fruit/999")
    assert response.status_code == 404
    assert b"Fruit not found" in response.data
