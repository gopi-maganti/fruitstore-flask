...
def test_update_fruit_info_success(client, add_fruit):
    ...
    response = client.put(
        f"/fruit/update/{fruit_id}", json={"price": 12.5, "quantity": 20}
    )
    assert response.status_code == 200
...
def test_add_fruit_missing_fields(client):
    ...
    response = client.post(
        "/fruit/add", data={**data, **file}, content_type="multipart/form-data"
    )
...
def test_get_all_fruits_exception(client):
    with patch.object(
        fruit_service, "get_all_fruits", side_effect=Exception("DB failure")
    ):
        response = client.get("/fruit/all")
        assert response.status_code == 500
...
def test_get_fruit_by_id_exception(client):
    with patch.object(
        fruit_service, "get_fruit_by_id", side_effect=Exception("lookup failed")
    ):
...
def test_search_fruits_value_error(client):
    with patch.object(
        fruit_service, "search_fruits", side_effect=ValueError("invalid filter")
    ):
...
def test_search_fruits_exception(client):
    with patch.object(
        fruit_service, "search_fruits", side_effect=Exception("search fail")
    ):
...
def test_delete_fruit_exception(client):
    with patch.object(
        fruit_service, "delete_fruits", side_effect=Exception("delete fail")
    ):
