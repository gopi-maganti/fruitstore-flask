...
def test_add_order_success(client, setup_order_data):
    ...
    response = client.post(f"/order/place/{data['user_id']}", json={
...
def test_get_order_by_user_id_success(client, setup_order_data):
    data = setup_order_data
    client.post(f"/order/place/{data['user_id']}", json={
        "cart_ids": [data["cart_id"]]
    })
...
def test_add_order_invalid_user_id(client, setup_order_data):
    data = setup_order_data
    response = client.post("/order/place/9999", json={
        "cart_ids": [data["cart_id"]]
    })
...
def test_get_order_history_exception(client):
    with patch.object(order_service, "get_order_history", side_effect=Exception("Fetch failed")):
...
def test_get_all_orders_exception(client):
    with patch.object(order_service, "get_all_orders", side_effect=Exception("Global fetch fail")):
        response = client.get("/order/all")
        assert response.status_code == 500
