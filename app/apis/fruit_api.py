...
def add_fruit_with_info() -> tuple[jsonify, int]:
    """
    Adds a new fruit along with its information and image.

    Accepts multipart/form-data with fruit details and image file.
    Validates input, checks image type, uploads image to S3, stores fruit data in the database.

    Returns:
        tuple: JSON response with status code.
    """
...
def get_all_fruits() -> tuple[jsonify, int]:
    """
    Retrieves all fruits and their associated information.

    Returns:
        tuple: JSON list of fruit details and HTTP status code.
    """
...
def get_fruit_by_id(fruit_id: int) -> tuple[jsonify, int]:
    """
    Retrieves a fruit and its info by fruit ID.

    Args:
        fruit_id (int): ID of the fruit.

    Returns:
        tuple: JSON object with fruit and info or error message, and status code.
    """
...
def search_fruits() -> tuple[jsonify, int]:
    """
    Searches fruits using keyword and numeric filters (price, quantity, weight).

    Query params:
        - search: str (name or color)
        - value: float (exact match)
        - price_min, price_max
        - weight_min, weight_max
        - total_quantity_min/max
        - available_quantity_min/max

    Returns:
        tuple: JSON list of matching fruits and status code.
    """
...
def update_fruit_info(fruit_id: int) -> tuple[jsonify, int]:
    """
    Updates selective fields in fruit info such as price, weight, sell_by_date, and quantity.

    Args:
        fruit_id (int): ID of the fruit to update.

    Returns:
        tuple: JSON object with updated info and status code.
    """
...
def delete_fruits() -> tuple[str | jsonify, int]:
    """
    Deletes multiple fruits and related data based on provided fruit IDs in the request body.

    Returns:
        tuple: Success message or error with appropriate HTTP status code.
    """
