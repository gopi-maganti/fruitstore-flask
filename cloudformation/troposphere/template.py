...
igw = template.add_resource(
    ec2.InternetGateway("FruitstoreIGW", Tags=Tags(*common_tags))
)

template.add_resource(
    ec2.VPCGatewayAttachment("AttachGateway", VpcId=Ref(vpc), InternetGatewayId=Ref(igw))
)

route_table = template.add_resource(
    ec2.RouteTable("PublicRouteTable", VpcId=Ref(vpc), Tags=Tags(*common_tags))
)
...
template.add_resource(
    ec2.SubnetRouteTableAssociation(
        "Subnet1RouteAssoc", SubnetId=Ref(subnet1), RouteTableId=Ref(route_table)
    )
)
...
key_pair = template.add_resource(
    ec2.KeyPair(
        "FruitstoreKeyPair",
...
instance_type_param = parameters.get("InstanceType")
instance_type_value = Ref(instance_type_param) if instance_type_param else default_instance_type
...
# ===== Output Public IP =====
template.add_output(
    Output("PublicIP", Description="Public IP of EC2", Value=Ref(ec2_instance))
)
